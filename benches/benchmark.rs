...
pub fn benchmark(c: &mut Criterion) {
    ...
    c.bench_function("day16", |b| {
        b.iter(|| {
            let mut input = day16::parse_input(include_str!("../inputs/day16.txt"));
            day16::part_1(black_box(&mut input));
            day16::part_2(black_box(&mut input));
        })
    });

    c.bench_function("day17", |b| {
        b.iter(|| {
            let input = day17::parse_input(include_bytes!("../inputs/day17.txt"));
            day17::part_1(black_box(input.clone()));
            day17::part_2(black_box(input));
        })
    });
    ...
    c.bench_function("day21", |b| {
        b.iter(|| {
            let input = day21::parse_input(include_str!("../inputs/day21.txt"));
            let solved = day21::solve(&input);
            day21::part_1(black_box(&input), black_box(&solved));
            day21::part_2(black_box(solved));
        })
    });

    c.bench_function("day22", |b| {
        b.iter(|| {
            let input = day22::parse_input(include_str!("../inputs/day22.txt"));
            day22::part_1(black_box(input.clone()));
            day22::part_2(black_box(input));
        })
    });
}
